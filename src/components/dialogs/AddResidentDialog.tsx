...
const AddResidentDialog = ({ open, onOpenChange, onAdd }: AddResidentDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert formData to match Resident type
      const residentData: Omit<Resident, 'id' | 'created_at'> = {
        name: formData.name,
        apartment_id: parseInt(formData.apartment) || 0,
        status: formData.status,
        contact_number: formData.contact,
        email: formData.email,
        resident_id: 0, // This will be generated by the database
        // Compatibility fields
        contact: formData.contact,
        apartment: formData.apartment
      };
      
      const success = await onAdd(residentData);
      if (success) {
